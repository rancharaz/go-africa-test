...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const signup = useMutation({
    mutationFn: (user: User) => {
      return axios.post(`${import.meta.env.VITE_SERVER_API_URL}/signup`, user)
    },
    onSuccess() {
      navigate("/login")
    },
  ...
  const login = useMutation({
    mutationFn: (id: string) => {
      return axios
        .post(`${import.meta.env.VITE_SERVER_API_URL}/login`, { id })
        .then(res => {
  ...
  const logout = useMutation({
    mutationFn: () => {
      return axios.post(`${import.meta.env.VITE_SERVER_API_URL}/logout`, { token })
    },
    onSuccess() {
      setUser(undefined)
      setToken(undefined)
